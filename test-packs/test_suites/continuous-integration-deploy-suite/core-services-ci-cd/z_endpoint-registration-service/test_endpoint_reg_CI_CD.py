...
def load_test_data(hostIpAddress, rabbitMqConnection):
...
def test_registerServiceWithNoAddress(rabbitMqConnection):
    with pytest.raises(NoMessageConsumedException):
        ...
        cleanup(service_id)
        rabbitMqConnection.bindQueueWithKey(endpointExchange,'testQueue', 'dell.cpsd.endpoint.discovered')


        #add service to consul
        test_serivce = {"ID": "testService1", "Name": "testService",
                        "Tags": ["testTag1"], "Check": {"HTTP":  rabbitMqConnection.url, "Interval": "5s"}}
        ...
        rabbitMqConnection.consumeMessageFromQueue('testQueue')
# # *******************************************************************************************
#
@pytest.mark.core_services_mvp_extended
def test_registerServiceWithNoPort(rabbitMqConnection):
    with pytest.raises(NoMessageConsumedException):
        # Testing that even tho Consul accepts a registration with no Address field, the Endpoint Registry
        # does not advertise its existence
        ...
        service_id="testService1"
        cleanup(service_id)
        rabbitMq.bindQueueWithKey(endpointExchange,'testQueue', 'dell.cpsd.endpoint.discovered')
...
@pytest.mark.core_services_mvp_extended
def test_registerServiceWithNoHealthCheck(rabbitMqConnection):
    with pytest.raises(NoMessageConsumedException):
        # Testing that even tho Consul accepts a registration with no Address field, the Endpoint Registry
        # does not advertise its existence
...
@pytest.mark.core_services_mvp_extended
def test_registerServiceSuccess(rabbitMqConnection):
    ...
    #add service to consul
    test_service = {"ID": service_id, "Name": "testService", "Address": "1.2.3.4", "Port": 200,
                    "Tags": ["testTag1"], "Check": {"HTTP":  rabbitMqConnection.url, "Interval": "2s"}}
    status_code = registerServiceWithConsul(test_service)
    assert status_code == 200, "The Register Service task was unsuccessful"
    ...
    message = rabbitMqConnection.consumeMessageFromQueue('testQueue')
    errors_list = []
...
@pytest.mark.core_services_mvp_extended
def test_registerTwoServicesSameName(rabbitMqConnection):
    ...
    regData1 = {"ID": "duplicateService1", "Name": "duplicateService","Address": "3.3.3.3", "Port": 300,
                "Tags": ["duplicate1"], "Check": {"HTTP":  rabbitMqConnection.url, "Interval": "2s"}}
    ...
    regData2 = {"ID": "duplicateService2", "Name": "duplicateService","Address": "4.4.4.4", "Port": 301,
                "Tags": ["duplicate2"], "Check": {"HTTP":  rabbitMqConnection.url, "Interval": "2s"}}
...
@pytest.mark.core_services_mvp
@pytest.mark.core_services_mvp_extended
def test_deregisterServiceSuccess(rabbitMqConnection):
    ...
    rabbitMqConnection.bindQueueWithKey(endpointExchange,'testQueue', 'dell.cpsd.endpoint.unavailable')
...
@pytest.mark.core_services_mvp_extended
def test_deregisterOneOfTwoSameServices(rabbitMqConnection):
...
@pytest.mark.core_services_mvp_extended
def test_deregisterTwoOfTwoSameServices(rabbitMqConnection):
# This test verifies what happens when all instances of a service (in this case 2) are deregistered
    ...
    regData1 = {"ID": "duplicateService1", "Name": "duplicateService", "Address": "3.3.3.3", "Port": 300,
                "Tags": ["duplicate1"], "Check": {"HTTP": rabbitMq.url, "Interval": "2s"}}
    status_code = registerServiceWithConsul(regData1)
    rabbitMqConnection.consumeMessageFromQueue('testQueue')
    assert status_code == 200, "The Register Service task was unsuccessful"
    ...
    regData2 = {"ID": "duplicateService2", "Name": "duplicateService","Address": "4.4.4.4", "Port": 403,
                "Tags": ["duplicate2"], "Check": {"HTTP": rabbitMq.url, "Interval": "2s"}}
    status_code = registerServiceWithConsul(regData2)
    rabbitMqConnection.consumeMessageFromQueue('testQueue')
    assert status_code == 200, "The Register Service task was unsuccessful"
