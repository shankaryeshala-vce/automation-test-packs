...
def getComplianceDataSystem(product, family, identifier, deviceFamily, deviceModel, deviceType, compDataFilename, sysUUID):
    ...
    if len(compData["devices"]) != "":
        ...
        assert compData["convergedSystem"]["systemUuid"] == deviceData["systemUuid"] == systemUUID, "Response shows incorrect System UUID."
        ...
        assert compData["convergedSystem"]["modelFamily"] == data["system"]["definition"]["modelFamily"], "Unexpected Model returned."
        assert compData["convergedSystem"]["product"] == data["system"]["definition"]["product"], "Unexpected Product returned."
        assert compData["convergedSystem"]["identifier"] == data["system"]["identity"]["identifier"], "Unexpected Identifier returned."
        assert compData["convergedSystem"]["serialNumber"] == data["system"]["identity"]["serialNumber"], "Unexpected Serial No. returned."
        ...
        while deviceIndex < totalDevices:
            if compData["devices"][deviceIndex]["uuid"] == deviceData["components"][compIndex]["uuid"]:
                ...
                assert compData["devices"][deviceIndex]["parentGroupUuids"][0] == deviceData["components"][compIndex]["parentGroupUuids"][0], "Response not detail parent Group UUID."
                assert "productFamily" in compData["devices"][deviceIndex]["elementData"], "Response not detail Product Family."
                ...
                assert "elementType" in compData["devices"][deviceIndex]["elementData"], "Response not detail ElementType."
                ...
                if compData["devices"][deviceIndex]["elementData"]["modelFamily"] == deviceFamily:
                    assert compData["devices"][deviceIndex]["uuid"] == deviceData["components"][compIndex]["uuid"], "Response detailed an empty group UUID."
                    assert compData["devices"][deviceIndex]["parentGroupUuids"][0] == deviceData["components"][compIndex]["parentGroupUuids"][0], "Response not detail parent Group UUID."
                    assert compData["devices"][deviceIndex]["elementData"]["elementType"] == deviceType, "Response not detail Element Type."
                    assert compData["devices"][deviceIndex]["elementData"]["model"] == deviceModel, "Response not detail Model."
                    assert compData["devices"][deviceIndex]["elementData"]["elementType"] == deviceData["components"][compIndex]["identity"]["elementType"], "Response not detail Element Type."
                    assert compData["devices"][deviceIndex]["elementData"]["model"] == deviceData["components"][compIndex]["definition"]["model"], "Response not detail Model."
                    assert compData["devices"][deviceIndex]["elementData"]["modelFamily"] == deviceData["components"][compIndex]["definition"]["modelFamily"], "Response not detail Identifier."
                    assert compData["devices"][deviceIndex]["elementData"]["productFamily"] == deviceData["components"][compIndex]["definition"]["productFamily"], "Response not detail Product Family."
                    assert compData["devices"][deviceIndex]["elementData"]["identifier"] == deviceData["components"][compIndex]["identity"]["identifier"], "Response not detail Identifier."
        ...
        while subIndex < totalSubComponents:
            ...
            if identifier in compData["subComponents"][subIndex]["elementData"]["identifier"]:
                if compData["subComponents"][subIndex]["parentDeviceUuid"] == compData["devices"][index]["uuid"]:
                    ...
                    assert compData["subComponents"][subIndex]["parentDeviceUuid"] == compData["devices"][index]["uuid"], "Response not detail parent Group UUID."
