...
def load_test_data():
    ...
    messageManu = af_support_tools.get_config_file_property(config_file=payload_file, heading=payload_header, property=payload_dataManu)
    xmessageManu = af_support_tools.get_config_file_property(config_file=payload_file, heading=payload_header, property=payload_xdataManu)
    messageAddendum = af_support_tools.get_config_file_property(config_file=payload_file, heading=payload_header, property=payload_dataAddendum)
    messageAddendum2 = af_support_tools.get_config_file_property(config_file=payload_file, heading=payload_header, property=payload_dataAddendum2)
    messageAddendum3 = af_support_tools.get_config_file_property(config_file=payload_file, heading=payload_header, property=payload_dataAddendum3)
    xmessageAddendum = af_support_tools.get_config_file_property(config_file=payload_file, heading=payload_header, property=payload_xdataAddendum)
    messageOriginal = af_support_tools.get_config_file_property(config_file=payload_file, heading=payload_header, property=payload_dataOriginal)
    xmessageOriginal = af_support_tools.get_config_file_property(config_file=payload_file, heading=payload_header, property=payload_xdataOriginal)
    vxrackMessageOriginal = af_support_tools.get_config_file_property(config_file=payload_file, heading=payload_header, property=payload_vxrackDataOriginal)
    vxrackMessageManu = af_support_tools.get_config_file_property(config_file=payload_file, heading=payload_header, property=payload_vxrackDataManu)
    vxrackMessageAddendum = af_support_tools.get_config_file_property(config_file=payload_file, heading=payload_header, property=payload_vxrackDataAddendum)
    vxrackMessageAddendum2 = af_support_tools.get_config_file_property(config_file=payload_file, heading=payload_header, property=payload_vxrackDataAddendum2)
...
def deleteTestQueues(testRequest, testResponse):
    ...
    af_support_tools.rmq_purge_queue(host=host, port=port, rmq_username=rmq_username, rmq_password=rmq_password, queue=testRequest, ssl_enabled=False)
    af_support_tools.rmq_purge_queue(host=host, port=port, rmq_username=rmq_username, rmq_password=rmq_password, queue=testResponse, ssl_enabled=False)
...
def insertDummyRCMRequest(payLoad, requestFile, responseFile):
    ...
    af_support_tools.rmq_bind_queue(host=host, port=port, rmq_username=rmq_username, rmq_password=rmq_password, queue='testInsertRCMDefinitionsRequest', exchange='exchange.dell.cpsd.rfds.rcm.definition.request', routing_key='#', ssl_enabled=False)
    af_support_tools.rmq_bind_queue(host=host, port=port, rmq_username=rmq_username, rmq_password=rmq_password, queue='testInsertRCMDefinitionsResponse', exchange='exchange.dell.cpsd.rfds.rcm.definition.response', routing_key='#', ssl_enabled=False)

    af_support_tools.rmq_publish_message(host=host, port=port, rmq_username=rmq_username, rmq_password=rmq_password,
                                         exchange="exchange.dell.cpsd.rfds.rcm.definition.request", routing_key="dell.cpsd.rfds.rcm.definition.request",
                                         headers=messageHeaderRequest, payload=payLoad, payload_type='json', ssl_enabled=False)

    time.sleep(2)
    my_request_body = af_support_tools.rmq_consume_message(host=host, port=port, rmq_username=rmq_username, rmq_password=rmq_password, queue='testInsertRCMDefinitionsRequest', ssl_enabled=False)
    my_response_body = af_support_tools.rmq_consume_message(host=host, port=port, rmq_username=rmq_username, rmq_password=rmq_password, queue='testInsertRCMDefinitionsResponse', ssl_enabled=False)
...
def verifyPublishedAttributes(filename, eolDate, numContents, train, version, family, model, option):
    ...
    if requestData["rcmDetails"]["rcmContents"] != '':
        assert (len(requestData["rcmDetails"]["rcmContents"])) == numContents, "Unexpected number of content entries found."
...
def verifyConsumedAttributes(filename, expectedResult, train, version, family, model, option):
    ...
    if responseData["correlationId"] != "":
        if responseData["insertSuccessful"] != "":
            ...
            assert len(responseData["insertSuccessful"]["rcmDetails"]["rcmContents"]) > 0, "Unexpected insert result found."
...
def verifyCorrectCorrelationID(requestFile, responseFile):
    ...
    if requestData["correlationId"] != "":
        if responseData["correlationId"] != "":
            ...
            assert requestData["correlationId"] == responseData["correlationId"], "Correlation ID on response message does not match."
...
def test_verifyPublishedAttributes1():
    verifyPublishedAttributes(path + 'insertRCMRequest1.json', "March 2019", 66, "1.2", "1.2.33", "Vblock", "340", "MANUFACTURING")
@pytest.mark.rcm_fitness_mvp
@pytest.mark.rcm_fitness_mvp_extended
def test_verifyPublishedAttributes2():
    verifyPublishedAttributes(path + 'insertRCMRequest2.json', "March 2019", 66, "1.2", "1.2.33", "VxBlock", "340", "MANUFACTURING")
@pytest.mark.rcm_fitness_mvp
@pytest.mark.rcm_fitness_mvp_extended
def test_verifyPublishedAttributes3():
    verifyPublishedAttributes(path + 'insertRCMRequest3.json', "March 2020", 66, "1.2", "1.2.33.1", "Vblock", "340", "ADDENDUM")
@pytest.mark.rcm_fitness_mvp
@pytest.mark.rcm_fitness_mvp_extended
def test_verifyPublishedAttributes4():
    verifyPublishedAttributes(path + 'insertRCMRequest4.json', "June 2021", 66, "1.2", "1.2.33.2", "Vblock", "340", "ADDENDUM")
@pytest.mark.rcm_fitness_mvp
@pytest.mark.rcm_fitness_mvp_extended
def test_verifyPublishedAttributes5():
    verifyPublishedAttributes(path + 'insertRCMRequest5.json', "Sept 2022", 66, "1.2", "1.2.33.3", "Vblock", "340", "ADDENDUM")
@pytest.mark.rcm_fitness_mvp
@pytest.mark.rcm_fitness_mvp_extended
def test_verifyPublishedAttributes6():
    verifyPublishedAttributes(path + 'insertRCMRequest6.json', "June 2020", 66, "1.2", "1.2.33.1", "VxBlock", "340", "ADDENDUM")
@pytest.mark.rcm_fitness_mvp
@pytest.mark.rcm_fitness_mvp_extended
def test_verifyPublishedAttributes7():
    verifyPublishedAttributes(path + 'insertRCMRequest7.json', "March 2019", 66, "1.2", "1.2.33", "Vblock", "340", "ORIGINAL")
@pytest.mark.rcm_fitness_mvp
@pytest.mark.rcm_fitness_mvp_extended
def test_verifyPublishedAttributes8():
    verifyPublishedAttributes(path + 'insertRCMRequest8.json', "March 2019", 66, "1.2", "1.2.33", "VxBlock", "340", "ORIGINAL")
@pytest.mark.rcm_fitness_mvp
@pytest.mark.rcm_fitness_mvp_extended
def test_verifyPublishedAttributes9():
    verifyPublishedAttributes(path + 'insertRCMRequest9.json', "March 2019", 37, "9.2", "9.2.33", "VxRack", "FLEX", "ORIGINAL")
@pytest.mark.rcm_fitness_mvp
@pytest.mark.rcm_fitness_mvp_extended
def test_verifyPublishedAttributes10():
    verifyPublishedAttributes(path + 'insertRCMRequest10.json', "March 2019", 37, "9.2", "9.2.33", "VxRack", "FLEX", "MANUFACTURING")
@pytest.mark.rcm_fitness_mvp
@pytest.mark.rcm_fitness_mvp_extended
def test_verifyPublishedAttributes11():
    verifyPublishedAttributes(path + 'insertRCMRequest11.json', "March 2020", 37, "9.2", "9.2.33.1", "VxRack", "FLEX", "ADDENDUM")
@pytest.mark.rcm_fitness_mvp
@pytest.mark.rcm_fitness_mvp_extended
...
def test_verifyConsumedAttributes10():
    verifyConsumedAttributes(path + 'insertRCMResponse10.json', True, "9.2", "9.2.33", "VxRack", "FLEX", "MANUFACTURING")
@pytest.mark.rcm_fitness_mvp
@pytest.mark.rcm_fitness_mvp_extended
