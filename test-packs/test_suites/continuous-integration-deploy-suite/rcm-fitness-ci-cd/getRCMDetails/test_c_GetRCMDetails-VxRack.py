...
def load_test_data():
    ...
    message = af_support_tools.get_config_file_property(config_file=payload_file, heading=payload_header, property=payload_data)
    print("Message: %s" % message)
    messageInvalidCorrID = af_support_tools.get_config_file_property(config_file=payload_file, heading=payload_header, property=payload_dataInvalidCorrID)
    print("MessageInvalidCorrID: %s" % messageInvalidCorrID)
    messageIP = af_support_tools.get_config_file_property(config_file=payload_file, heading=payload_header, property=payload_dataIP)
    messageInvalidRoutingKey = af_support_tools.get_config_file_property(config_file=payload_file, heading=payload_header, property=payload_dataInvalidRoutingKey)
    messageInvalidUUID = af_support_tools.get_config_file_property(config_file=payload_file, heading=payload_header, property=payload_dataInvalidUUID)
    messageNoCorrID = af_support_tools.get_config_file_property(config_file=payload_file, heading=payload_header, property=payload_dataNoCorrID)
    messageNoHostname = af_support_tools.get_config_file_property(config_file=payload_file, heading=payload_header, property=payload_dataNoHostname)
    messageNoKey = af_support_tools.get_config_file_property(config_file=payload_file, heading=payload_header, property=payload_dataNoKey)
    messageNoUUID = af_support_tools.get_config_file_property(config_file=payload_file, heading=payload_header, property=payload_dataNoUUID)
    messageAllEmpty = af_support_tools.get_config_file_property(config_file=payload_file, heading=payload_header, property=payload_dataAllEmpty)
    messageSpaces = af_support_tools.get_config_file_property(config_file=payload_file, heading=payload_header, property=payload_dataSpaces)
    messageVxrack = af_support_tools.get_config_file_property(config_file=payload_file, heading=payload_header, property=payload_dataVxRack)
...
def deleteTestQueues(testRequest, testResponse):
    ...
    propsRequest = pika.BasicProperties(headers=messageReqHeader, content_type='application/json', content_encoding='UTF-8')

    messageResHeader = {'__TypeId__': 'com.dell.cpsd.rcm.definition.service.api.RcmDefinitionsDetailsMessage'}
    propsResponse = pika.BasicProperties(headers=messageResHeader, content_type='application/json', content_encoding='UTF-8')
    connection = pika.BlockingConnection(parameters)
    channel = connection.channel()
    af_support_tools.rmq_purge_queue(host=host, port=port, rmq_username=rmq_username, rmq_password=rmq_password, queue=testRequest)
    af_support_tools.rmq_purge_queue(host=host, port=port, rmq_username=rmq_username, rmq_password=rmq_password, queue=testResponse)
...
def getRCMRequest(payLoad, requestFile, responseFile):
    ...
    af_support_tools.rmq_bind_queue(host=host, port=port, rmq_username=rmq_username, rmq_password=rmq_password,
								queue='testGetRCMDetailsRequest', exchange='exchange.dell.cpsd.rfds.rcm.definition.request',
								routing_key='#')
    af_support_tools.rmq_bind_queue(host=host, port=port, rmq_username=rmq_username, rmq_password=rmq_password,
								queue='testGetRCMDetailsResponse', exchange='exchange.dell.cpsd.rfds.rcm.definition.response',
								routing_key='#')
    ...
    time.sleep(2)
    my_request_body = af_support_tools.rmq_consume_message(host=host, port=port, rmq_username=rmq_username, rmq_password=rmq_password, queue='testGetRCMDetailsRequest')
    my_response_body = af_support_tools.rmq_consume_message(host=host, port=port, rmq_username=rmq_username, rmq_password=rmq_password, queue='testGetRCMDetailsResponse')
